...
;#define AppProductVersion "5.0.0-beta"
#endif
...
; Require Windows 10 1607 or newer (minimum needed for .NET 10 ("10-1607-e-lts")).
; Ref(s): https://github.com/dotnet/core/blob/main/release-notes/10.0/supported-os.md and https://jrsoftware.org/ishelp/index.php?topic=winvernotes
...
begin
  if CurPageID = wpReady then begin
    ...
    if (not IsDotNetInstalled('Microsoft.AspNetCore.App', '10.0.0', 'x64') OR not IsDotNetInstalled('Microsoft.AspNetCore.App', '10.0.0', 'x86')) then begin
        DownloadPage.Add('https://aka.ms/dotnet/10.0/dotnet-hosting-win.exe', 'dotnet-hosting-win.exe', '');
    end;

    if (not IsDotNetInstalled('Microsoft.WindowsDesktop.App', '10.0.0', 'x64')) then begin
        DownloadPage.Add('https://aka.ms/dotnet/10.0/windowsdesktop-runtime-win-x64.exe', 'windowsdesktop-runtime-win-x64.exe', '');
    end;

    if not IsDotNetInstalled('Microsoft.WindowsDesktop.App', '10.0.0', 'x86') then begin
        DownloadPage.Add('https://aka.ms/dotnet/10.0/windowsdesktop-runtime-win-x86.exe', 'windowsdesktop-runtime-win-x86.exe', '');
    end;
